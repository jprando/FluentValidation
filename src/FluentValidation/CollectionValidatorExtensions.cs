...
namespace FluentValidation {
	...
	public static class CollectionValidatorExtensions {
		...
		public static IRuleBuilderOptions<T, IEnumerable<TCollectionElement>> SetCollectionValidator<T, TCollectionElement>(this IRuleBuilder<T, IEnumerable<TCollectionElement>> ruleBuilder, IValidator<TCollectionElement> validator) {
			var childValidator = new InlineValidator<IEnumerable<TCollectionElement>>();
			var childRule = childValidator.RuleForEach(x => x).SetValidator(validator);
			ruleBuilder.SetValidator(childValidator);

			// needed to support DependentRules
			...
			return new CollectionValidatorRuleBuilder<T, TCollectionElement>(ruleBuilder, childRule, parentValidator);
		}
		...
		public static ICollectionValidatorRuleBuilder<T, TCollectionElement> SetCollectionValidator<T, TCollectionElement, TValidator>(this IRuleBuilder<T, IEnumerable<TCollectionElement>> ruleBuilder, Func<T, TValidator> validator)
			...
			var childValidator = new InlineValidator<IEnumerable<TCollectionElement>>();
			var childRule = childValidator.RuleForEach(x => x).SetValidator((coll, ctx) => validator((T) ctx.InstanceToValidate));
			ruleBuilder.SetValidator(childValidator);
		...
		private class CollectionValidatorRuleBuilder<T,TCollectionElement> : ICollectionValidatorRuleBuilder<T,TCollectionElement>, IExposesParentValidator<T> {
			...
			IRuleBuilderOptions<IEnumerable<TCollectionElement>, TCollectionElement> adaptor;

			public CollectionValidatorRuleBuilder(IRuleBuilder<T, IEnumerable<TCollectionElement>> ruleBuilder, IRuleBuilderOptions<IEnumerable<TCollectionElement>, TCollectionElement> adaptor, IValidator<T> parent) {
			...
			public IRuleBuilderOptions<T, IEnumerable<TCollectionElement>> SetValidator<TValidator>(Func<T, IValidationContext, TValidator> validatorProvider) where TValidator : IValidator<IEnumerable<TCollectionElement>> {
				return ruleBuilder.SetValidator(validatorProvider);
			}
			...
			public ICollectionValidatorRuleBuilder<T, TCollectionElement> Where(Func<TCollectionElement, bool> predicate) {
				...
				adaptor.When(x => predicate((TCollectionElement)x));
				return this;
			}
